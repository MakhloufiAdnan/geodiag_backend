...
const registrationLimiter = rateLimit({
	...
	max: 5, 
	message: "Trop de tentatives d'inscription depuis cette IP, veuillez réessayer dans une heure.",
    standardHeaders: true, // Return rate limit info in the `RateLimit-*` headers
	legacyHeaders: false, // Disable the `X-RateLimit-*` headers
});

// Route publique, mais avec une validation stricte des données d'entrée.
router.post('/register/company', registrationLimiter, validateRegister, registrationController.registerCompany);

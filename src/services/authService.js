...
     */
    async loginCompanyAdmin(email, password) {
        ...
        const refreshToken = generateRefreshToken({ userId: user.user_id, familyId });
        const decodedRefreshToken = jwt.decode(refreshToken);
        const expiresAt = new Date(decodedRefreshToken.exp * 1000);

        await refreshTokenRepository.create(user.user_id, refreshToken, familyId, expiresAt);
    ...
    async refreshTokens(oldRefreshToken) {
        ...
        if (!storedToken) {
            try {
                ...
                throw new ForbiddenException('Tentative de réutilisation de jeton détectée. Session révoquée.');
            } catch (error) {
        ...
        const accessToken = generateAccessToken({ userId: user.user_id, role: user.role });
        const newRefreshToken = generateRefreshToken({ userId: user.user_id, familyId });
        const decodedRefreshToken = jwt.decode(newRefreshToken);
        const expiresAt = new Date(decodedRefreshToken.exp * 1000);

        await refreshTokenRepository.create(user.user_id, newRefreshToken, familyId, expiresAt);
